...
def main():
  ...
  with col1:
    ...
    top_product = filtered_df.groupby('product').agg({
      'total_sales': 'sum',
      'quantity': 'sum'
  ...
  with col2:
    ...
    sales_manager = filtered_df.groupby('manager')['total_sales'].sum().sort_values(ascending=False).reset_index()
    fig_manager = px.bar(
      sales_manager,
      x='manager',
      y='total_sales',
